...
test_image = './sample_image/muti.jpg'
...
multiplier = [x * model_['boxsize'] / oriImg.shape[0]
              for x in param_['scale_search']]

print("xxxxx:", multiplier)

heatmap_avg = torch.zeros((len(multiplier), 19,
                           oriImg.shape[0],
                           oriImg.shape[1])).to(device)
...
for m in range(len(multiplier)):
    ...
    feed = Variable(torch.from_numpy(imageToTest_padded)).to(device)
    output1, output2 = model(feed)

    heatmap = nn.functional.interpolate(output2,
                                        (oriImg.shape[0], oriImg.shape[1]),
                                        mode='bilinear',
                                        align_corners=True).to(device)
    
    paf = nn.functional.interpolate(output1,
                                    (oriImg.shape[0], oriImg.shape[1]),
                                    mode='bilinear',
                                    align_corners=True).to(device)
...
heatmap_avg = torch.transpose(
    torch.transpose(
        torch.squeeze(
            torch.mean(heatmap_avg, 0)), 0, 1), 1, 2).to(device)
paf_avg = torch.transpose(
    torch.transpose(
        torch.squeeze(
            torch.mean(paf_avg, 0)), 0, 1), 1, 2).to(device)
